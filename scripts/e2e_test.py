...
def main():
    ...
    in_path = f"/Volumes/{args.catalog}/{args.schema}/inbox/e2e-{token}.md"
    out_dir = f"/Volumes/{args.catalog}/{args.schema}/deliverables"
    ...
    # 1) PUT: drop a unique document into the inbox (the 'S3 upload').
    ...
    # 3) GET + ASSERT: the expected output must exist in the outbox and reference our input.
    matches = [e for e in w.files.list_directory_contents(out_dir)
               if e.path.rsplit("/", 1)[-1].startswith(out_prefix)]
    if not matches:
        print(f"\nRESULT: FAIL - no output file '{out_prefix}*' found in outbox")
        return 1
