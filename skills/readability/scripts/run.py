"""
Skill entrypoint for readability, per the uniform skill contract (issue #16).
...
scripts/analyze.py, exact Flesch reading-ease / grade-level metrics) with the LLM half
(the runner's retrying inside-Databricks client, grounded in those exact metrics), then
writes the same combined report byte-for-byte as the pre-#16 runner did.
